...
Baseline observations use overlapping 12-week forward windows, since every week in the dataset is tested rather than only non-overlapping periods, meaning adjacent baseline observations share most of their forward window and are not fully independent of each other. Fisher's exact test assumes independent observations in each cell of the contingency table, so this overlap means the resulting p-values should be read as an approximate, directional guide to significance rather than an exact figure. This does not appear to change the substantive conclusion, since the positioning-based results are far from significance under this test across every parameter combination tried, but it is a real limitation of the current design rather than a fully resolved statistical question.
...
The baseline is computed across every week in the dataset, including the weeks later flagged as positioning or price events, rather than excluding them. This means the baseline is best read as "the general population of all weeks" compared against a flagged subset, not a fully independent control group, a design choice that if anything makes it harder, not easier, to find a difference between events and baseline.
